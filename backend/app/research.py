...
_QUOTE_RULE = (
    ...
    "严禁用文献列表前的编号(如 [11]、[12]、[13])来指代文献——每一处引用都必须写成上面这样"
    "完整、可点击的 Markdown 链接(带作者、年份与真实 URL)，绝不能只留一个方括号里的裸数字。"
)
...
def _parse_json(raw: str, opener: str, closer: str):
    ...
    try:
        ...
        log_swallow("找选题: LLM 输出无法解析为 JSON(将走默认兜底)", exc)
        return None
...
def _reduce_messages_deep(field: str, summaries: list[tuple[str, str]], trials_note: str) -> list[dict]:
    system = (
        ...
        "- **新颖性说明（对照检索结果，指出是否已有相近工作、本选题不同在哪）**；"
        "- 相关文献链接（逐条写成完整的 [第一作者 et al., 年份](真实URL) 链接，不要用 [12]、[13] 这种裸编号）；\n"
        "- **自评分（帮助非专业读者快速判断）**：单列一行，格式严格为 "
        "`> 可行性 ★N/5（一句话理由）｜创新性 ★N/5（一句话理由）`，N 为 1-5 整数，"
        "理由要基于上文证据与子方向空白，不要套话。\n"
        "## 四、首选推荐\n给出你最推荐的一个并说明理由（可结合上面的自评分）。\n\n"
        "铁律：只能引用上面小结中出现过的文献链接，严禁编造任何文献、作者或链接；"
        "每一处引用都必须是完整的 [第一作者 et al., 年份](真实URL) 链接(原样复制小结中的链接, 含支持句 title)，"
        "严禁用 [11]、[12] 这类裸编号指代文献；证据不足时明说‘现有检索结果有限’。"
    )
...
def _synthesis_messages(field: str, papers: list[dict]) -> list[dict]:
    system = (
        ...
        "每个课题用 `### 候选选题N：一句话题名` 作小标题，分点给出：拟解决的科学问题、创新点、可行性、"
        "相关文献链接（逐条写成完整的 [第一作者 et al., 年份](真实URL) 链接，不要用 [12]、[13] 这种裸编号），"

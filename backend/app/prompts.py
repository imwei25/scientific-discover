...
# 全局反幻觉前缀: 任何面向用户的生成任务, 都在 system prompt 顶部注入这段, 明确
# "信息不足则声明缺失、不编造具体数值/位置/文献"; 避免各模块提示词质量参差。
# 用户/评审多次反馈"AI 编造章节位置/编造 p 值/编造引用文献"是最大信任杀手, 这里做统一兜底。
ANTI_FAB = (
    "【铁律·反幻觉】(所有输出必须遵守):\n"
    "① 只依据用户实际提供的材料回答; 材料不足处, 明确写 [需研究者确认] 或 [材料未提供]; 严禁编造\n"
    "② 不得编造具体数值(p 值/样本量/效应量/剂量/百分比等); 引用材料中确有的数值时保持原样\n"
    "③ 不得编造章节位置(如见方法学 2.3 节)、页码、图表编号; 若需要指路只能引用材料里确有的锚点\n"
    "④ 引用材料原文时用中文书名号「」包裹, 便于作者核对; 释义/概括则不加书名号\n"
    "⑤ 不得编造文献(作者/期刊/年份/DOI); 若讨论需要引用外部文献, 只列 [待补充引用] 占位\n"
)


def _prefix_anti_fab(system: str) -> str:
    """在 system prompt 顶部注入反幻觉铁律; 若原文已有 铁律 或 反幻觉 字样则原样返回。"""
    if "铁律" in system or "反幻觉" in system:
        return system
    return ANTI_FAB + "\n" + system
...
def build_checklist(inputs: dict) -> list[dict]:
    ...
    system = (
        ...
        "『正文位置/证据』列必须用中文书名号「」引用原文原句(至少 10 个字符)，"
        "否则该行状态一律标 ⚠️不充分（防止编造章节/页码/编号）；"
...
def build_format(inputs: dict) -> list[dict]:
    ...
    system = (
        ...
        "格式与可编译性约束（为了下游能一键生成 LaTeX/Word）：\n"
        "• 不使用 emoji 或装饰性符号（💡🗺️📊 等），必要处用文字描述；\n"
        "• 章节标题只写标题文字本身，不要加『I.』『II.』『1.』这类编号前缀"
        "（期刊模板会自动编号，加了会双重编号）；\n"
        "• 表格必须用标准 markdown 表格（含分隔行 `|---|---|`），不要用 ASCII 画的伪表格；\n"
        "• 代码/命令统一放到围栏代码块 ```...``` 中；\n"
        "• 不要在正文中输出投稿元数据（如『Title』『Index Terms』这种作为章节名）。\n"

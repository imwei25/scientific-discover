...
| 基金标书 | `grant` | **grant-proposal·定标模式**(查当年申报通知并让用户确认，落 requirement_card.md，见表下注) → research-scan → topic-selection → **idea-forge**(立意锻打对话，默认做、可跳过，见表下注) → **novelty-check**(对锻打定稿的科学问题做新颖性裁定+预注册锁，见表下注) → grant-proposal(起草) → peer-review(自查) → render-pdf-doc |
...
- **grant 第 0 步「定标模式」与 `requirement_card.md`（全流水线公共输入）**：标的要求决定后面每一步——文体家族、评审评价维、指南方向、经费档位。所以第 0 步先定渠道、**用户没提供文件就联网查当年申报通知/指南（内置渠道也查，通知才管得住改版与指南方向），查到的候选必须编号列给用户确认**（无人值守才自动选官方来源并显著标注未确认），落 `requirement_card.md` 即停。下游消费：research-scan 把扫描范围限定在受理方向内；topic-selection 评分加 funder-fit 维；idea-forge 阶段 0 从卡取评审评价维定议程权重、取指南方向做对表；grant-proposal 起草不再问渠道。用户**单步直呼** grant-proposal 起草时不强制走定标（技能内部照旧自载卡）。
- **选题不由 AI 遴选（2026-08-18 定的产品立场）**：用户给了自己的**研究问题 / 痛点**（grant 模块表单必填这项，或对话里说了）时，research-scan 以它为锚做调研（不自拟替代方向），topic-selection 走**细化模式**——候选 1 固定"按你写的原样推进"，其余是结合文献的细节调整变体（收窄人群 / 换结局 / 改设计……，每条带真实引文），选哪条恒由用户拍板；AI 自拟课题只在用户什么都没给时兜底。细则见两技能 SKILL.md。
- **grant / paper 里的 `idea-forge`（立意锻打对话）**：成文前用多轮对话把立意/创新点/方案（标书）或故事线/主张/目标刊（论文）锻硬——**开工第一轮先摸底**（把本场景的决策节点排成清单让用户一次性自答，填掉的直接销账），剩下的节点里：用户有自己的答案就基于他的答案拷问，空手了才检索发散给带引文的候选；**任何检索先过三闸**（值不值查 / 查什么先给用户过目 / 用户点名的只核实不泛查），唯一不可省的是"首次·没人做过"类会写进正文的断言；产出 `design_brief.md`（+标书场景的 `closest_work.md`），是 grant-proposal 第 3.5 步 / write-paper 引言与讨论的直接输入。**grant 里默认做**（标书空洞的根因就是论证没锻过），两种情况可跳：目录里已有完整 `design_brief.md`；或用户赶时间明说跳过（此时 grant-proposal 第 3.5 步自行补检索）。**paper 里可选**，三种情况才做：用户没想好讲什么故事/投哪；write-paper 发现核心主张含糊或明显 overclaim；用户主动要求被拷问。它是对话式技能，用户不在线（无人值守）时整步跳过。**锻打内可按需升级 deep-research**：承重墙节点（最接近工作拥挤/矛盾、机制链承重环判不清、gap 归因选"证据互斥"）快筛撑不住时深挖一发（每场 ≤2 发、先问用户、AUTO 自动采推荐；细则见 idea-forge SKILL.md「深挖升级」节）——deep-research 在 grant 里是锻打的弹药库，不单列流程步。
- **两条起点（与模块界面的「从哪里开始」分叉同款）**：用户明说**已有大致 idea / 已有初稿**时，别把人拉回从零。标书已有立意 → 跳过 research-scan 与 topic-selection，直接从 idea-forge 锻打他的设想起步（其余步骤与闸不变；**进场先做入口对表深检**：用 deep-research 对他的设想查领域真实进展与在研竞争、落 `idea_landscape.md`，用户点头才跑、无人值守自动做、不占锻打深挖配额——自带的设想往往几年没对过表；判「已被回答」时没有选题步可退，当场给 2–3 个带引文的转向候选让用户挑定再继续）。论文已有初稿 → write-paper 以初稿为底本逐节补强改写（不另起炉灶重写，改动要能对得回原稿），literature-review 降为可选补强（引用撑不住主张时仍回来补做），reference-check / peer-review 等质量闸照过。
...
- **发到手机（微信/企微）**：`push-chat`（"把这个文件发我微信 / 推到企微 / 发到我手机上"——经本机聊天接入把文件/消息推到用户已接入的微信、企业微信，任何会话可用；失败时先自愈（重启桥）再给出用户指令；仅 Windows 桌面版且软件开着；个人微信有每日发送额度，仅在用户明确要求时推）
...
- **Python 统一走项目根 `.venv`，且路径一律加引号**：`"${REPO_ROOT:-/app}/.venv/bin/python"`。
  引号不是可选的——桌面版装在 `.../Niuma Science/bundle/app`，**目录名里有空格**，不加引号
  bash 会从空格处把路径切成两半，报 `.../Local/Niuma: No such file or directory`。
  **看到这个报错不是"没装 Python / 没有 .venv"，是漏了引号**：补上引号重跑即可。
  环境是随安装包 / 镜像装好的，**任何情况下都不要重建 `.venv` 或重装 requirements**
  （白烧十几分钟，还可能把包版本弄坏）。自建部署 / 开发机首次初始化走仓库根的
  `install.ps1`（Windows）/ `install.sh`。

...
PATTERNS = [
    ...
    # 军官证中文写法："军字第88991号"/"警字第123号"，无"军官证"标签也认（label+num 两组，与上一行统一处理）。
    ("军官证", re.compile(r"([军警空海]字第)(\d{3,8}号)"), None, "MIL"),
    # 无标签的床位自然写法："内科3床""3床""CCU2床"——准位置标识。要求"床"后缀，误伤低。
    ("床位", re.compile(r"(?<![一-龥0-9])[一-龥A-Za-z]{0,4}\d{1,3}床(?![位])"), None, "BED"),
...
# 复姓：命中时名字可到 4 字（欧阳娜娜）。放在单姓之前优先匹配，避免"欧"被当单姓只吃 2 字。
_COMPOUND = ("欧阳|司马|诸葛|上官|司徒|夏侯|皇甫|尉迟|公孙|慕容|长孙|宇文|司空|独孤|南宫|"
             "端木|东方|钟离|令狐|轩辕|澹台|公冶|宗政|濮阳|淳于|单于|太叔|申屠|夏候|赫连|"
             "闻人|东郭|百里|呼延|东门|西门|上官|万俟")
# 姓名核心：复姓+1~2字 或 单姓+1~2字。
_NAME_CORE = (r"(?:(?:" + _COMPOUND + r")[一-龥]{1,2}|[" + _SURNAMES + r"][一-龥]{1,2})")

# 称谓触发词（前置）：临床文本里的人名几乎都有这类前缀。命中"称谓+姓名"才脱，精度高、不伤术语。
_APPEL = (r"患者|病人|患儿|家属|陪护|陪人|代诉人|主诉人|联系人|监护人|委托人|受试者|供者|受者|"
          r"主任医师|副主任医师|主治医师|主治医生|经治医师|管床医师|接诊医师|主刀医师|麻醉医师|"
          r"住院医师|副主任护师|主管护师|责任护士|护士长|护士|护师|医师|医生|大夫|技师|主任|教授|"
          r"其父|其母|其子|其女|其夫|其妻|其兄|其弟|其姐|其妹|其孙|"
          r"女儿|儿子|子女|配偶|丈夫|妻子|爱人|父亲|母亲|哥哥|弟弟|姐姐|妹妹|孙子|外孙")
# 强职称后缀（后置）：姓名紧跟这些词时，人名在前、称谓在后（"孙悦护士""李明医师"）——中文病历常见语序。
_TITLE_SUFFIX = (r"主任医师|副主任医师|主治医师|主管护师|责任护士|护士长|医师|医生|护士|护师|"
                 r"大夫|教授|技师|主任|院长")

# 前置称谓 + （容忍"说自己叫/名叫/自称"等短引名短语）+ 姓名
_NAME_RE = re.compile(
    r"(?P<appel>" + _APPEL + r")"
    r"(?P<sep>(?:[（(]?(?:说自己叫|自称为|自称|名叫|叫做|叫|姓名|系|为|是|：|:|，|,)?[）)]?[\s]?){0,2})"
    r"(?P<name>" + _NAME_CORE + r")"
)
# 后置职称：姓名 + 强职称后缀（逆序写法）
_NAME_SUFFIX_RE = re.compile(r"(?P<name>" + _NAME_CORE + r")(?=(?:" + _TITLE_SUFFIX + r"))")


# 3 字名末位若是这些字，几乎不是给定名用字、却常作紧邻词的词首（电话/联系/主诉/病史/
# 查房/入院/出院/收入/住院/门诊/手术/诊治…）——说明贪婪多吃了一字，回退为 2 字名并把该字还回文本。
# 注意：只放几乎【不】用作给定名的字，否则会把真 3 字名(张建平/李晓丽)的末字误剪。
# 剔除了 平/治/手/术/来/素/主 等常见名字用字，仅留明确的临床词首字。
_NAME_TAIL_STOP = set("电话联系诉病住门床身年性无现的为是在于及与和即因经已未曾"
                      "查房既往述陪症说家")
...
def mask_names(text, mask, hits):
    ...
    text = _NAME_RE.sub(_sub_pre, text)
    text = _NAME_SUFFIX_RE.sub(_sub_suf, text)
    return text

...
def main():
    ...
    # ★ 同时回 raw（表头原文，按列序）与 dupes（真重名）。headers 里的"（重名 N）""（第 N 列·无列名）"
    #   是【给人看的显示名】，在真实表里根本不存在 —— 界面若拿它当值送给脚本，按它取列必然取不到。
    #   显示名与真列名必须分开回，与 workflows.mjs 的 parseHeaders 同口径。
    seen, headers, raw = {}, [], []
    for i, h in enumerate(list(df.columns)):
        orig = _s(h)
        raw.append(orig)
        base = orig or "（第 %d 列·无列名）" % (i + 1)
        k = base.lower()
        seen[k] = seen.get(k, 0) + 1
        headers.append("%s（重名 %d）" % (base, seen[k]) if seen[k] > 1 else base)
    # 真重名只算【完全同名】的：仅大小写不同的两列（Age / age）本来就是两个合法且不同的列名
    _cnt = {}
    for h in raw:
        if h:
            _cnt[h] = _cnt.get(h, 0) + 1
    dupes = [h for h, n in _cnt.items() if n > 1]

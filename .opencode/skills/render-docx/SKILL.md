...
- **块级空行补齐（默认开，最先跑）**：pandoc 要求 pipe 表格**前面有空行**。而写作阶段极常见地把表题贴着表格写（`**表1 …**` 下一行直接 `| 列 | 列 |`），此时 pandoc 把整张表当成表题那一段的"懒续行"——**表被摊平成纯文本、docx 里一个 `<w:tbl>` 都没有，且全程不报错**。标题、列表贴着正文写也一样会被吞。渲染前先跑 `normalize_md_blocks.py` 补空行（只增删空行、不改任何一行内容、围栏代码块内不碰），关闭用 `--no-normalize`。

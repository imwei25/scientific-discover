...
4. **多源增强检索**（`references/enhanced_search.py`）——需要**更全的召回**时（交叉学科、
   ...
   - **⚠ 词表覆盖：每个概念都要把「规范名 + 习用名」并列成 OR 组。** 检索源按题录字面匹配，
     一个概念只写一种说法就会整片漏掉只用另一种说法的文献，**而且在输出里完全看不出来** ——
     命中数照样几千条，证据表照样很长，只是该引的那几篇不在里面。实测：写
     `GLP-1 receptor agonist` 命中 8787 条，却匹配不上 SMART 试验（*Nat Med* 2025，题录里
     只有 `semaglutide`）；`marginal zone lymphoma AND KMT6A` 只有 1 条，换 `EZH2` 有 853 条。
     开跑前逐个概念自问：**药物**类名与主要通用名并列了吗；**基因/蛋白**官方符号、旧符号、
     常见别名并列了吗；**疾病**全称、缩写、旧命名并列了吗。拿不准某个别名是否在用，
     先各跑一次 `--limit 1` 看命中数。细则与更多案例见
     `literature-review/references/search-rounds.md` 同名小节（两条检索路径共用这套规矩）。
   - **⚠ OR 组里的多词词条必须自己加括号**：`(GLP-1 receptor agonist OR semaglutide)` 会被按
     词级结合解析、布尔逻辑散架。脚本的合法性闸会自动补括号并告警，但**最恶毒的地方在于
     命中数是涨的**（实测 7101 → 16410 而目标文献丢了）—— 所以"命中变多"**绝不能**当成
     "召回变宽了"，那往往正是检索式写坏了的信号。写成 `((GLP-1 receptor agonist) OR semaglutide)`
     一次写对，别依赖闸兜底。

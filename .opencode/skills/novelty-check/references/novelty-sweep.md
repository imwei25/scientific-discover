...
### 最接近的 5–10 篇
| # | PMID/DOI | 标题（短） | 年 | 设计 | N | 与本题相关性 | **它在什么条件下失效（八类失配任选）** |
|---|---|---|---|---|---|---|---|

> 末列是给下游用的：人群 / 终点 / 时间尺度 / 机制层次 / 测量 / 设计 / 规模 / 一致性——**具体到设计参数**
> （"92% 为术后人群""主要终点为替代指标""n=40 对该效应量把握度仅 45%"），不写"研究较少""不够深入"。
> 分类定义见 `grant-proposal/references/rationale-and-innovation.md` §2；填了这一列，标书的立项依据主体段就已经写好一半。

### 它们留下了什么没回答
[2–4 句：现有文献没回答、而本题回答的是什么]

### 这个空白为什么至今还在（四选一）
[ ] 技术不可及　[ ] 数据不可得　[ ] 概念未提出　[ ] 做过但证据互斥
> 与上面的裁定不是一回事：裁定判"新不新"，这一问判"为什么还新着"。它决定下游标书的创新点类型
> 与可行性怎么论（详见 `rationale-and-innovation.md` §3）。答不出来 → 多半是检索还不够，或这个空白其实没价值。

...
>
> **⚠ 词表覆盖：每个概念都要把「规范名 + 习用名」并列成 OR 组，不能只给一个说法。**
> 检索源是按题录字面匹配的，一个概念只写一种说法，就会整片漏掉只用另一种说法的文献 ——
> **而且这种漏法在检索输出里完全看不出来**：命中数照样几千条，证据表照样很长，
> 只是该引的那几篇不在里面。两个实测案例：
> - **药物：类名 ≠ 通用名。** 写 `GLP-1 receptor agonist` 命中 8787 条，但 SMART 试验
>   （Apperloo, *Nat Med* 2025，非糖尿病 CKD 人群唯一的专属肾脏结局 RCT）**一条都匹配不上** ——
>   它的题录里只有 `semaglutide`，没有这个类名。真机综述因此写出"非糖尿病 CKD 只有替代终点
>   证据"这种审稿人一眼能看出的硬伤。**正确写法**：
>   `"(GLP-1 receptor agonist) OR semaglutide OR tirzepatide OR liraglutide OR dulaglutide"`。
> - **基因：官方符号 ≠ 文献习用名。** `marginal zone lymphoma AND KMT6A` 只命中 **1** 条，
>   换成 `EZH2` 是 **853** 条 —— KMT6A 是 EZH2 的 HGNC 现行符号，但绝大多数文献仍写 EZH2。
>   **正确写法**：`"EZH2 OR KMT6A OR PRC2"`。
>
> 开跑前对着检索概念逐个自问一遍：**药物**有没有把类名与该类主要通用名并列；**基因/蛋白**
> 有没有把官方符号、旧符号、常见别名并列；**疾病**有没有把全称、缩写、旧命名并列
> （如 `MALT lymphoma OR (extranodal marginal zone lymphoma) OR MZL`）。
> 拿不准某个别名是否真在用，就先各跑一次 `--limit 1` 看命中数 —— 一条式几秒钟，
> 比事后被 reference-check 或审稿人打回来便宜得多。
>
> **⚠ OR 组里的多词词条必须自己加括号**：`(GLP-1 receptor agonist OR semaglutide)` 这种写法
> 会被按词级结合解析、布尔逻辑散架。脚本有合法性闸会自动补括号并告警，但**最恶毒的地方
> 在于命中数是涨的**（实测 7101 → 16410，而目标文献丢了）—— 所以看到"命中变多"
> **绝不能**当成"召回变宽了"，那往往正是检索式写坏了的信号。写成
> `((GLP-1 receptor agonist) OR semaglutide)` 一次写对，别依赖闸兜底。
>

...
### ⚠️ 人工核对小节的写法是【固定格式】，不要自由发挥
`integrity_report.md` 里凡是"需要作者核对"的条目，**必须**写成下面两者之一——网关的质量闸靠它判断
这一步该不该标红，而本技能的铁律又禁止你写"未通过 / 需返工"这类裁定语，**这个格式就是闸唯一能认的信号**：

```
## 需核对信号（N 项）        ← 小节标题里必须带【条数】，N 为整数
### ★1 <一句话说清是哪个文件/行/列的什么问题>
### ★2 …
```
- 条数写 0（`## 需核对信号（0 项）`）表示这一维度干净，闸会放行——**只有真干净时才这么写**。
- 别改用别的符号（`⭕`、`🔺`、纯数字编号）或漏掉条数：实测漏写会让闸【静默变绿】，
  而报告里明明列着 6 条硬性不自洽（含生理不可能的 eGFR=1220、死亡数 8 vs 9），
  用户在步骤条上却看到一个绿勾，导出的 Word 上也不会有任何提示。
- 闸标红时界面显示的是「**有待核信号**」而不是「需返工」——与 signal-not-verdict 一致，
  你不需要、也不应该为了让它变红去写指控性措辞。

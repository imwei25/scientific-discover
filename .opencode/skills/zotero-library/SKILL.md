---
name: zotero-library
description: 读取用户本机 Zotero 文献库并对其中 PDF 做带页码的全文证据检索（PaperQA2-lite），供带引用回答；默认只读，push 子命令可把题录写入 Zotero 当前分类。仅与 Zotero 同机时可用，探测失败回退 search-lit/fulltext-retrieval。触发："用我的 Zotero""我本地的文献库"。

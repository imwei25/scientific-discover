...
## 6-7. Shadow / Overlay / Stroke / Gradient / Pattern / Native markers

> **按需查，不必前置通读** —— 阴影与图片蒙版叠加、虚线 `stroke-dasharray`、`stroke-linejoin`、
> `text-decoration`、线性/径向渐变、`<pattern>` 图案填充、原生 PPTX 表格/图表标记
> (`data-pptx-native`)、`transform: rotate`、圆环/扇形弧路径、对角线上的多边形箭头 ——
> 配方全在 [`svg-effects.md`](svg-effects.md)。画到需要某一项时再打开对应小节。
>
> ⚠️ 上面的 §1 禁用清单与 §2 PPT 兼容替代**不在**那个文件里，仍以本文件为准。

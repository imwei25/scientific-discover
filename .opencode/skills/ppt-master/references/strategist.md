...
> **按需读** —— 图像来源选项表、AI 配图 h.5 的 rendering × palette 候选、点缀插图（spot illustration）
> 的判断与优先级、多图版式与 Pipeline handoff，全部在
> [`strategist-images.md`](strategist-images.md)。**确认阶段要定第 8 项（图像使用）时打开它**；
> 若这份稿件确定 `image_usage: none`（纯文字 deck），整段跳过、不必打开。
...
Save outputs to `<project_path>/design_spec.md`（`<project_path>` 见 SKILL.md 顶部约定，形如 `<项目名>_<format>_<YYYYMMDD>/`（相对当前工作目录））。

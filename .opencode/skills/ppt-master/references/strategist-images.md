# Strategist §h — Image Usage Confirmation (on-demand)

> 从 `strategist.md` 搬出，**按需读**：只有当确认阶段真的要定图像来源 / AI 配图风格时才打开。
> `image_usage: none` 的稿件（纯文字 deck）整份跳过 —— 那正是这一段最常见的归宿。
> 本文件是 §h 的**唯一权威**；`SKILL.md` 与 `strategist.md` 只留指针。

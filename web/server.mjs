...
//   skills    =「专项技能 / Skills」：单点能力，随时插进任一流程
// 表内顺序即两个页面的展示顺序（工作台分栏、聊天页欢迎区列表都读 /api/modules 的原序）。
// 将来新增模块若忘了写 group，工作台把它落到「专项技能」栏——不会从界面上凭空消失。
const MODULE_DEFS = {
  ...
  // 归「工作台」而不是「专项技能」：它不是插进某条流水线的单点能力，而是自成一件事——
  // 选个文件夹、读完、出台账、按类归档，从头到尾在这一个模块里闭环。位置排在自由对话之前。
  // 输入是【一个本机文件夹】而不是上传的文件：靠既有的「工作目录」机制（会话 cwd = 用户选的目录），
  // 所以这个模块不需要用户上传任何东西，也不占会话存储。
  litmanage: { name: "文献管理",       group: "workbench",
              desc: "选一个文件夹，自动读完里面的 PDF / Word（可勾上 Markdown、txt 或自填格式），按你给的标准（或 AI 自定口径）分类，出一张多 sheet 的 Excel 台账：文献一行含年份、作者、杂志与核心观点，非文献则列「主要内容」；可随时改分类，并一键把原文件按类归到子文件夹。" },
...
// 用户可见文案统一叫「全程托管」（界面 2026-08-18 从「自动推进」改的名，两边要对得上）
const AUTO_STOP_NOTES = {
  cap: () => `全程托管：已连续跑了 ${AUTO_MAX_ROUNDS} 轮仍未见完成标记，为防失控已暂停。请检查目前的产物后手动继续。`,
  stalled: () => "全程托管：连续两轮输出几乎相同（疑似原地打转），已暂停托管。请检查产物后手动继续。",
  quota: () => "全程托管：额度/积分已用尽，托管暂停；恢复后可手动继续（日额度每日 0 点(UTC) 重置）。",
  empty: () => "全程托管：本轮没有文本输出（多半是上游异常），托管暂停。",
  idle: () => "全程托管：连续两轮没有再动手做事，视为已经做完，托管收尾。若还有没做完的，直接说一句就能接着来。",
}

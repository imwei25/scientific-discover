  ...
  grant: {
    ...
    steps: [
      ...
      { id: "forge", name: "立意锻打", skill: "idea-forge", skillAlias: ["novelty-check"], gate: true,
        ...
        note: "两件事连着做完，顺序不能颠倒：先用 idea-forge 按其 SKILL.md 跑锻打对话（一轮只问一个问题、编号候选、每轮落盘 `forge_log.md`，产出 `design_brief.md` + `closest_work.md`），**再**对 design_brief 里定稿的那句关键科学问题跑 novelty-check（以 closest_work.md 为最接近文献表起点补严，不重做）。裁定完**在回话里点名说清档位（真新 / 增量 / 已被回答）与依据**；判「已被回答」退回「方向调研」换题，不许带着已被回答的题写标书。**无人值守（AUTO）时跳过锻打对话**，直接对用户选定的题做裁定 —— 本格靠裁定产物判完成，不会卡死。用户从「打磨已有」进来时（任务卡里「从哪里开始＝打磨已有」，设想在「你的研究设想」/「已有工作基础」/上传材料里），流程里没有前两格：锻打对象就是那段设想；判「已被回答」时没有「方向调研」可退 —— 当场给 2–3 个带引文的转向候选（换人群 / 换机制层 / 换结局），按编号让用户挑定再继续锻打，同样不许带着已被回答的题往下写。" },

...
const LITREAD_MODES = withAsk([
  chatMode("基于已传的文献", "对着这些文章随便问",
    "它读的是你左边传进来的那几篇；前面的导读、综合、对比、翻译都还在上下文里，可以接着问。"),
  ...
  // ---- 多篇模式：一次读【勾上的那几篇】，两种出结论的形式 ----------------------------
  // 【为什么是两个模式而不是一个带开关的模式】"合并"和"对比"要的东西根本不同：
  //   合并 = 把几篇证据并成一条总结论（哪些互相印证、合起来支持什么、证据强度如何）；
  //   对比 = 逐维度列差异（人群 / 干预 / 终点 / 效应量 / 结论各自是什么、为什么不一致）。
  // 合成一个模式靠一句话参数分流，用户点完得不到确定的形状，而两份产物本来就该各存各的文件
  //（面板靠 file 正则认正文，一个模式只能认一份）。所以拆成两格，各自一个 mark、一份产物。
  //
  // 【勾选从哪来】左上角文件条上每份文件一个复选框（source.pick），勾中的那几篇由前端展开成
  // {docs} 塞进提示词。没勾满两篇时 need:["docs:2"] 会把这一格拦在发出之前 ——
  // 发出去再让模型发现"只有一篇"，用户已经花掉一轮了。
  { id: "synth", label: "多篇综合", icon: "review", mark: "【多篇综合】", badge: "勾选多篇 · 合并出结论",
    need: ["docs:2"], needHint: "在左上角的文件条上，把要一起读的那几篇勾上（至少两篇）。",
    file: "^multi_synthesis.*\\.md$",
    out: "^(multi_synthesis|fulltext)[^/]*\\.(md|docx|pdf)$",
    // ★ 最后那句不是客套：实测合并模式保住的是主要终点与效应量，单篇的次级指标数字会被压掉
    //   （这是"合并"的应有之义，不是缺陷）。空面板上先说清，用户才知道细节该去哪一格拿。
    empty: ["还没有综合结论", "在左上角勾上要一起读的那几篇（≥2），点「开始」——它会逐篇抽正文，再合并成一条总结论。要某一篇的完整细节，去「文献导读」单独跑那一篇。"],
    tell: "把**勾选的那几篇**逐篇抽成 `fulltext_<短名>.md`（已经抽过的直接用，别重抽），"
      + "再**合并**成一条总结论写进 `multi_synthesis.md`，并用 `render-docx` 转出 `multi_synthesis.docx`",
    prompt: "请把我勾选的这几篇文献 {docs} **一起读**，出一份**合并结论**（不是逐篇摘要的拼盘）。\n\n"
      + "**第一步**：逐篇把原文抽成文本，每篇各存一份 `fulltext_<短名>.md`"
      + "（`<短名>` 用第一作者+年份或题名前几个词，能一眼认出是哪篇即可）。"
      + "会话里已经有对应 `fulltext*.md` 的，**直接读它，不要重抽**。\n\n"
      + "**第二步**：写 `multi_synthesis.md`，按这个结构：\n"
      // ★ 注册号写进纳入清单：实测（2026-09-21，两篇真实 RCT 对跑）合并模式会把注册号、
      //   次级指标的具体数字这类"单篇细节"压掉 —— 主要终点与效应量都在，注册号一个没剩。
      //   而注册号恰恰是这份清单最常被拿去做证据表的一列，点名要就能留住，代价只有一行。
      + "1. **纳入清单** —— 每篇一行：短名｜研究类型｜人群与样本量｜干预/暴露｜主要终点｜"
      + "核心结果（带关键数字）｜注册号（原文没有就写「原文未报告」）；\n"
      + "2. **一句话总结论** —— 这几篇合起来最可靠的那个结论是什么；\n"
      + "3. **互相印证的点** —— 哪些发现在 ≥2 篇里重复出现，各自出自哪篇（点名到篇 + 第几节/哪张图表）；\n"
      + "4. **互相矛盾或不一致的点** —— 列清楚分歧在哪、可能的来源（人群/剂量/终点定义/统计方法/随访时长），"
      + "**不要强行调和**，调和不了就写「无法判定」；\n"
      + "5. **证据强度** —— 合起来能支撑到什么程度（设计等级、样本量、一致性、是否有共同的偏倚方向）；\n"
      + "6. **共同的空白** —— 这几篇都没回答的问题；\n"
      + "7. **对读者课题的意义**。\n\n"
      + "硬要求：**每一条结论后面都要标出处（哪一篇 + 第几节/哪张图表）**，标不出来的就别写；"
      + "数字一律照抄原文，原文没写的写「原文未报告」；"
      + "**不许引入这几篇之外的文献，也不许拿背景知识补**；这几篇里只有一篇提到的事实要写明「仅 X 篇报告」。\n\n"
      + "**最后**：用 `render-docx` 转一份 Word：\n"
      + "```bash\n"
      + "bash \"${REPO_ROOT:-/app}/.opencode/skills/render-docx/scripts/render_docx.sh\" "
      + "-i multi_synthesis.md -o multi_synthesis.docx --journal generic-submission\n"
      + "```\n"
      + "md 与 docx 都要留；转换失败就如实说明并照常交付 md，别假装出了 Word。" },
  { id: "compare", label: "多篇对比", icon: "diff", mark: "【多篇对比】", badge: "勾选多篇 · 逐维度对照",
    need: ["docs:2"], needHint: "在左上角的文件条上，把要对比的那几篇勾上（至少两篇）。",
    file: "^multi_compare.*\\.md$",
    out: "^(multi_compare|fulltext)[^/]*\\.(md|docx|pdf)$",
    empty: ["还没有对比结果", "在左上角勾上要对比的那几篇（≥2），点「开始」——它会逐维度把几篇摆在一张表里比。"],
    tell: "把**勾选的那几篇**逐篇抽成 `fulltext_<短名>.md`（已经抽过的直接用），"
      + "再**逐维度对照**写进 `multi_compare.md`（**必须有一张一篇一列的对照表**），"
      + "并用 `render-docx` 转出 `multi_compare.docx`",
    prompt: "请把我勾选的这几篇文献 {docs} 放在一起**逐维度对比**（这一格要的是差异，不是合并结论）。\n\n"
      + "**第一步**：逐篇把原文抽成文本，每篇各存一份 `fulltext_<短名>.md`"
      + "（`<短名>` 用第一作者+年份或题名前几个词）。已经有对应 `fulltext*.md` 的**直接读，不要重抽**。\n\n"
      + "**第二步**：写 `multi_compare.md`：\n"
      + "1. **对照表**（markdown 表格，**一篇一列**，行是维度）：研究问题｜研究设计｜时间与地点｜"
      + "人群与纳排｜样本量｜干预/暴露｜对照｜主要终点及其定义｜随访时长｜统计方法｜主要效应量（含 95%CI、p）｜"
      + "主要结论｜注册号｜资金与利益冲突｜局限。原文没报告的格子一律写「原文未报告」，**不许空着、更不许推测**；\n"
      + "2. **关键差异逐条说明** —— 表里哪几行差得最要紧，差异会怎样影响结果的可比性；\n"
      + "3. **结论一致还是冲突** —— 逐个主要终点判：一致 / 部分一致 / 冲突 / 不可比（终点定义不同就是不可比），"
      + "每一条写明判断依据；\n"
      + "4. **谁的证据更强** —— 就每一处分歧说明哪一篇更可信及理由（设计、样本量、偏倚风险、终点是否硬终点）；"
      + "判不了就写「证据不足以判定」，**不要为了给结论而排座次**；\n"
      + "5. **合起来能说什么、不能说什么**。\n\n"
      + "硬要求：每个数字都要能指回具体某一篇的某一节/某张图表；不许跨篇串数据"
      + "（把 A 篇的样本量安到 B 篇上是这一格最容易出的错，写完自己回查一遍）；"
      + "**不许引入这几篇之外的文献**。\n\n"
      + "**最后**：用 `render-docx` 转一份 Word：\n"
      + "```bash\n"
      + "bash \"${REPO_ROOT:-/app}/.opencode/skills/render-docx/scripts/render_docx.sh\" "
      + "-i multi_compare.md -o multi_compare.docx --journal generic-submission\n"
      + "```\n"
      + "md 与 docx 都要留；转换失败就如实说明并照常交付 md。" },
  ...
  litread: {
    ...
    intake: [
      { id: "docFile", label: "文献原文", type: "files", required: true,
        ...
        help: "可以一次传好几篇：单篇照常做导读 / 翻译 / PPT，多篇可以勾选几篇一起「综合」或「对比」。"
            + "图片型扫描件会先走 OCR，识别不准的地方会如实标出来。" },
      LANG,
    ],
    steps: [
      { id: "ingest", name: "读入原文", skill: "fulltext-retrieval",
        ...
        emits: ["fulltext.md", "fulltext_*.md", "reading_guide.md", "reading_guide*.md", "reading_guide.docx",
                "translation_zh.md", "translation_zh.docx",
                // 多篇模式的两份产物。挂在这里的道理与上面那几个一样：artifactLine 只从
                // steps[].emits 收"产物用约定名"，名字漂了 reader 的面板就认不回正文。
                "multi_synthesis.md", "multi_synthesis.docx", "multi_compare.md", "multi_compare.docx"],
        render: "report",
    ...
    reader: {
      intro: {
        title: "文献研读",
        lead: "上传一篇或多篇 PDF / Word 文献，点「开始研读」——单篇先出一份导读，理清核心与论证逻辑；传了多篇还可以勾选几篇，一起出合并结论或逐维度对比。之后随时能对着原文追问、要全文翻译，或让它做一套汇报 PPT。",
        dropTitle: "点击或拖拽文献到此处",
        dropHint: "支持 PDF / Word（.pdf · .docx · .doc）　·　可一次传多篇",
        // 图标写死在文案旁边（{t,i}）：模式条的顺序调过之后，按位置取图标会让这几个小标签集体错位
        startText: "上传并进入",
        chips: [{ t: "文献导读", i: "guide" }, { t: "多篇综合", i: "review" }, { t: "多篇对比", i: "diff" },
          { t: "对着原文追问", i: "chat" }, { t: "全文翻译", i: "translate" }, { t: "汇报 PPT", i: "ppt" }],
        tip: "传多篇时，左上角每份文件前面有个勾选框，勾上哪几篇，「多篇综合 / 多篇对比」就只读哪几篇。<br>"
          + "图片型扫描件会先走 OCR，识别不准的地方会如实标出来。<br>所有结论只依据你传的这些原文——原文没写的，它会写「原文未报告」，不会替你补。",
      },
      // multi：一次可以传多篇（文件条上一份一枚 chip）。pick：chip 上再多一个勾选框，
      // 勾中的那几篇由前端展开成 {docs} —— 「多篇综合 / 多篇对比」读的就是它。
      source: { kind: "doc", field: "docFile", multi: true, pick: true,
        accept: ".pdf,.docx,.doc,.odt", exts: ["pdf", "docx", "doc", "odt"] },
      ...
      chat: { placeholder: "对着左边这些文献随便问，例如：第 3 组的样本量是多少？",
        firstTurn: "我上传了文献 {docs}，请先按前言把要用到的那几篇抽成文本再回答。我的问题是：" },
      modes: LITREAD_MODES,
    },
    ...
    flow: `\n- **本模块 = 研读【用户上传的这一篇 / 这几篇】文献**，不检索、不找别的文献、不写综述。用户问的一切都以这些原文为准。`
      // 多篇是后加的，而"这一篇"的说法散在下面每一条里。这一条单独把范围讲清楚：
      // 哪个模式读一篇、哪个模式读一组，以及【产物别互相覆盖】——五篇各跑一次导读却都写进
      // reading_guide.md 的话，用户回头只看得见最后一篇那份，而界面上五轮都打着绿勾。
      + `\n- **一次可以传多篇**。哪个模式读几篇，看用户消息里给了什么：`
      + `\`【文献导读】\`/\`【全文翻译】\`/\`【演示 PPT】\` 只读用户点名的【那一篇】；`
      + `\`【多篇综合】\`/\`【多篇对比】\` 读用户在消息里列出的【那几篇】，`
      + `**列了哪几篇就只读哪几篇**，别把会话里其余的文献也捎上（用户是特意勾的）。`
      + `\n- **多篇时产物要带短名后缀，不许互相覆盖**：正文 \`fulltext_<短名>.md\`、`
      + `导读 \`reading_guide_<短名>.md\`、译文 \`translation_zh_<短名>.(md|docx)\`；`
      + `\`<短名>\` 用第一作者+年份或题名前几个词，同一篇在所有产物里用同一个短名。`
      + `只有一篇时沿用不带后缀的旧名（\`fulltext.md\` / \`reading_guide.md\` / \`translation_zh.md\`）。`
      + `**多篇综合 / 对比的产物固定叫 \`multi_synthesis.md\` / \`multi_compare.md\`，不加后缀**（界面按这两个名字找正文）。`
      ...
      + `丢了还不报错。抽好的正文按上面那条命名规则落盘（单篇 \`fulltext.md\`、多篇 \`fulltext_<短名>.md\`），`
      + `**本会话后续所有模式都直接读已经抽好的那份，同一篇不要抽第二遍**——`
      + `先 \`ls\` 看一眼会话目录里有没有对应的 \`fulltext*.md\`，有就直接用。`

...
> （开发者发布新版本：设好签名密钥 `$env:TAURI_SIGNING_PRIVATE_KEY="$env:USERPROFILE\.tauri\research-assistant.key"`（注意变量名没有 `_PATH`，值可以是路径或密钥内容），
> `cargo tauri build` 后跑 `scripts\publish-release.ps1`；已装 v3.0.0+ 的用户启动时会收到自动更新提示。）

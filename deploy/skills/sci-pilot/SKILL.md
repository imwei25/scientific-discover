---
name: sci-pilot
description: 医学科研请求的**统一入口与调度中心（主控优先）**。本套件所有科研 / 医学写作类请求都先经本技能：判意图 → 定范围 → 派发。**单步任务**（画一张图 / 查一个 DOI / 脱敏 / 把 md 转 PDF / 做一次统计……）本技能轻量直派给对应单项技能（只多"判一下意图"这一步）；**完整目标**（写综述 / 论文 / 标书、系统综述、从零做课题、把数据做成投稿、"继续上次那个课题"）走对应流水线（调研→选题→标书/论文→检索→分析→作图→查引用→去AI味→排版）、在 workspace/ 维护状态、可中断续跑。当用户提出任何科研写作 / 分析 / 检索 / 排版 / 评审 / 选题 / 调研类请求时都先用本技能。
license: MIT
metadata:
  version: "1.0"
---
...
## 本技能是统一入口（主控优先：所有请求先经这里判意图、再派发）
本套件采用**主控优先**：科研/医学写作类请求都先由本技能判意图、定范围、再派发。宁可单步任务多一步，也保持入口统一、可控、可留痕。据范围走两条路：
- **单步且明确**（只画一张图、只查一个 DOI、只脱敏、只把 md 出 PDF、只做一次统计/一次评审……）→ **轻量直派**：不建 workspace、不走全流水线，判明意图后**直接调对应单项技能**做完即可（对用户只多"判一下意图"这一步）。
- **完整目标**（从零到成稿/成书、把数据做成投稿、"续上次那个课题"）→ 走下面 §一 的流水线，建 workspace/manifest、维护状态、可中断续跑。
- **判不准是单步还是完整目标** → 问用户一句（如"只要我做这一步，还是从头把整个流程走完？"）。
> **派发标记（防兜圈的机械信号，必写）**：派发任何技能时——**含单步直派、也含流水线每一步**——先写一行标记：`[sci-pilot派发·<单步 或 流水线第N步>·<技能名>·直接执行]`。各单项技能的"范围自检"闸凭这行标记识别"已被派发→直接做、不回绕"；只有**没有这行标记、且是用户冷启动直接触发**时，它们才把请求交回本技能。这样即便单步不建 workspace，被派发技能也有可核查的硬信号，不会误判成冷启动而兜圈。

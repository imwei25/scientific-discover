...
3. **用 venv 的解释器装依赖**（从这步起就写死用 `.venv`，不再碰系统 Python）：
   - Windows：`.venv/bin/python -m pip install -U pip -r scripts/requirements-skills.txt`

...
## 范围自检（开工第一件事）
- **只要做其中某一步**（如"帮我对这批文献双人筛选/去重""只做一次 RoB2/GRADE"）→ **就地做那一步**，别硬拉全流程。
- **要"做完整个系统综述并写成投稿稿" + 工作区状态/续跑** → 可建议走 `sci-pilot` 的 review 流水线（它把本技能的方法学流程 → `write-paper` 成文串起来、维护 workspace、可续跑）。
> 本技能是方法学本体；默认按用户要的粒度做，别把 sci-pilot 变成必经收费站。

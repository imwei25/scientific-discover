...
- **若本次不是 `sci-pilot` 派发来的**（用户冷启动直接触发、sci-pilot 还没介入）→ **先交回 `sci-pilot`** 判意图与范围。它判为**单步**（如"只对这批文献双人筛选/去重""只做一次 RoB2/GRADE"）会立刻派回来、你再就地做那一步；判为**完整目标**（做完整个系统综述并写成投稿稿）就走 review 流水线（本技能方法学八步 → `write-paper` 成文、维护 workspace、可续跑）。
- **若本次已是 `sci-pilot` 在派发你**（有 workspace 上下文，或 sci-pilot 刚决定要做系统综述/其中某步）→ **直接做，别再往回绕**（防兜圈）。

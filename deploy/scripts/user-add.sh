#!/usr/bin/env bash
# 新增用户：分配端口 → 生成强密码 → 写 users/<name>.env → 重渲染 compose → 创建(不启动)容器 → 同步 Caddy/DuckDNS → 热加载 manager。
...
tmpl="${DOMAIN_TEMPLATE:-weigu-{name}.duckdns.org}"
domain="${tmpl//\{name\}/$name}"
...
LAN_PASSWORD=$pass
EOF
echo "写入 $env（端口 $port，域名 $domain）"

scripts/render-compose.sh
docker compose up --no-start "agent-${name}"
scripts/caddy-sync.sh   || echo "!! Caddy 同步失败，请检查 scripts/caddy-sync.sh 与 /etc/caddy 权限"
scripts/duckdns-sync.sh || echo "!! DuckDNS 同步失败：请先到 DuckDNS 面板手动创建子域 ${domain%.*}（免费最多 5 个），再重跑 scripts/duckdns-sync.sh"

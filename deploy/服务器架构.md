...
| 技能 | `COPY .opencode/skills/ → /app/.opencode/skills/`（zotero-library 由 .dockerignore 排除），主控指令 `/app/AGENTS.md`（COPY 仓库根 `AGENTS.md`） |
| 内部端口 | 网关 3000、OpenCode 4098 |

> 技能只有**一个源头** `.opencode/skills/`（deploy/skills 副本已删除），改一处即可；线上生效仍需重建镜像（`redeploy-skills.sh`）。

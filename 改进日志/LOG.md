...
- **D3 移动端**：本品是桌面本地应用(Tauri / 局域网桌面浏览器)，非移动场景，按第 15 轮判定不投入窄屏适配。
- **测试**：`npm run build` 通过；Playwright **48/48 全绿**；error 测试新增断言 result-error `role=alert` 与状态区 `aria-live=polite`；dist 已重建提交。
- **commit**：见本次提交
...
## 2026-06-28 — 可用性/帮助模块（loop 第16轮）：9 个子工具加"?"使用说明
- **动机**：经 14+ 轮迭代，产品已达 8 模块工作台 + 大量子工具；流程图/statcheck/随机化/DMP/知情同意/ZIP/关键词/图注/PICO 这 9 处对终端用户"何时用·如何用"不直观。
- **改动**：新增 `HelpModal.tsx`(三路关闭 ESC/×/遮罩 + 焦点回归 + body 滚动锁) / `HelpButton.tsx`(圆形"?" + 模态控制) / `helpContent.tsx`(9 条统一三段：何时使用 / 如何使用 / 最简示例, 真实数字示例 + 使用前提如知情同意"必经 IRB"/ZIP"依赖前置产出")；styles.css 末尾追加 ~90 行 modal/按钮样式(沿用现有 token, 无新色板)；5 个模块文件各挂 1–3 行 `<HelpButton helpKey="…" />`，无后端、无业务逻辑改动。
- **测试**：新增 `help-modal.spec.ts`(2 用例：模态三路关闭+焦点回到触发按钮+三段内容齐全 + 6 个静态可见按钮存在性)；Playwright 48 → 50 全绿；dist 已重建。零额度。
- **commit**：见本次提交
